...
double ForeignPerfBasket::computeCurrencyPerformance(const PnlMat* path, int currencyId, int numAssets) const
{
    ...
    double dateScale = (double) t1 * monitoringTimeGrid_->getMaturity() / monitoringTimeGrid_->getNumberOfDaysInYear();
    double t1Double  = t1 * dateScale;
    double t2Double = t2 * dateScale;
    ...
    for (size_t i = 0; i < assetCurrencyMapping_.size(); i++)
    {
        ...
        {
            sumT1 += MGET(path, t1, i) / (fxT1 * foreignInterestRates_.at(currencyId - 1)->discount(0.0, t1Double));
            sumT2 += MGET(path, t2, i) / (fxT2 * foreignInterestRates_.at(currencyId - 1)->discount(0.0, t2Double));
            count++;
        }
    }
